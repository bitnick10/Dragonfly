...
/// qi ming xing
class QStrategyQmx: public QStrategyBase {
public:
    QStrategyQmx(QStockMarket& market) : QStrategyBase(market) {}
    ...
    bool static IsQualified(const Stock& stock, year_month_day_hour_min_sec date) {
        if (stock.id()[0] == '6' || stock.id()[0] == '0') {
            ...
            if (true
                    ...
                    && stock.trade_data()[i - 1].body_amplitude() < 1.0 / 100
                    && stock.trade_data()[i - 2].is_stick_filled()

                    && stock.trade_data()[i].close > stock.trade_data()[i - 2].open
                    && stock.trade_data()[i - 1].close < stock.trade_data()[i - 2].close
                    && stock.trade_data()[i - 1].open <= stock.trade_data()[i - 2].close
                    //&& stock.trade_data()[i - 2].up_percent() < -2.0 / 100
                    ...
                    && stock.indicators().kdj()[i].j > stock.indicators().kdj()[i].k
    ...
    void Trading(const QStockMarket & market) {
        ...
        if (!account.positions.empty()) {
            ...
            for (auto && p : copyPositions) {
                //const auto& stock = market[p.stock_id];
